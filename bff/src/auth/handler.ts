...
export function registerAuthRoutes(app: FastifyInstance): void {
  // POST /auth/email/otp
  app.post('/auth/email/otp', async (req: FastifyRequest<{ Body: OtpRequestBody }>, reply: FastifyReply) => {
    ...
    const responsePayload: { message: string; code?: string } = { message: 'OTP sent successfully' };
  ...
  app.post('/auth/email/verify', async (req: FastifyRequest<{ Body: VerifyRequestBody }>, reply: FastifyReply) => {
