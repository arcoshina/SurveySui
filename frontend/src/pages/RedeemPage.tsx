...
export default function RedeemPage() {
  ...
  const ssrTreasuryId = import.meta.env.VITE_SSR_TREASURY_ID ?? ''
  ...
  // Fetch user's StackedSurveysuiReward (SSR) coins
      ...
      <div className="w-full">
        ...
        <div className="border-b pb-6 mb-8">
          ...
          <p className="mt-3 text-lg text-neutral-600 max-w-2xl">
            列出您持有的 `StackedSurveysuiReward` 憑證。選擇後呼叫合約將 SSR 憑證兌換成 `SurveysuiReward` (SR) 代幣。
          </p>
        </div>
          ...
          {!account && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 shadow-sm text-center">
              ...
              <p className="text-sm text-amber-700 mb-4 max-w-md mx-auto">
                要查看或兌換您的 StackedSurveysuiReward 憑證，您需要連接您的 Sui 錢包。
              </p>
          ...
          {txStatus === 'success' && txDigest && (
            <div role="status" className="bg-emerald-50 border border-emerald-200 text-emerald-800 p-6 rounded-xl shadow-sm space-y-2">
              ...
              <p className="text-sm text-emerald-700">
                您的 SSR 憑證已成功在 AMM 池中換回 SR 代幣，並已發送回您的錢包。
              </p>
          ...
          <div className="space-y-6">
            ...
            {isLoading ? (
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-2xl py-16 px-6 text-center shadow-inner">
                ...
                <h3 className="text-base font-bold text-gray-700">您目前沒有可兌換的 SSR 憑證</h3>
                <p className="text-sm text-gray-500 mt-2 max-w-sm mx-auto">
                  完成填寫問卷活動後，您會獲得 StackedSurveysuiReward 憑證。屆時它們將會顯示在此處以供您進行兌換。
                </p>
              </div>
            ) : (
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-1">
                          ...
                          <span className="text-3xl font-extrabold text-gray-900 tabular-nums">
                            {formatBalance(coin.balance)} <span className="text-lg font-bold text-blue-600">SSR</span>
                          </span>
                        </div>
                      </div>
