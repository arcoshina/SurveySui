...
```
per_response_sui = gas_compensation_amount + storage_compensation_amount
...
- `purge_grace_ms`：預設 **90 天**（`DEFAULT_PURGE_GRACE_MS`），creator 可調但下限 **7 天**（`MIN_PURGE_GRACE_MS`、`EGraceTooShort`）。
- 一致性：`survey.vault_id == vault.id` 且 `registry.registered_vaults[vault_id] == survey.id` 雙重驗證（`EInvalidSurveyVaultMatch`）。

**分批執行**：每次呼叫最多刪 `config.purge_answers_batch` 筆答案 dynamic field（預設 **100**，protocol admin 經 `set_purge_answers_batch` 調整，下限 1）。
...
| `PURGE_GAS_BUDGET_CAP_MIST` | 100_000_000 | purge gas budget 上限 |
