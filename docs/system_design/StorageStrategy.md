...
> **答卷一律 inline 上鏈，無 Walrus 路線。** 超過 inline 上限即**拒絕送出**（禁止大型答卷）；理由：(a) blob_id 鏈上不可驗、易被詐冒導致補助池抽乾，(b) 小 blob 編碼後仍頂 ~63 MiB 計價下限，正常答卷不會大到需要 Walrus。需附大型內容者由當事人自行傳雲端硬碟連結（連結極短，落在 inline 內）。

| 層 | 規則 | 來源 |
|----|------|------|
| 前端門檻 | 加密後 > `VITE_ANSWER_SIZE_THRESHOLD_KB`（預設 **6**，對齊鏈上 6144 bytes）→ 前端**阻擋送出**並顯示引導文案 | [`SurveyPage.tsx`](../../frontend/src/pages/SurveyPage.tsx)、`vite.config.ts` |
| 鏈上 inline | `encrypted_answers` ≤ `vault.max_inline_answer_bytes`：預設 **6144**、creator 可調範圍 **1024–32768**（`EInlineAnswerTooLarge` / `EMaxInlineOutOfRange`）。空答卷 `EEmptyAnswers` | `survey_vault.move` |
| BFF 代付 gate | claim 只驗 inline：`effectiveInlineLimit = min(env cap, vault 鏈上值)`，超限回 `inline_answer_too_large`；env：`MAX_INLINE_ANSWER_BYTES` / `MAX_INLINE_ANSWER_KB` | [`inlineLimit.ts`](../../bff/src/gas/inlineLimit.ts)、[`sponsorTxValidation.ts`](../../packages/gas-station-core/src/sponsorTxValidation.ts) |
| 代付政策 | inline 答卷照常走代付（額度、rate limit、dry-run 既有閘）；vault gas 池不足回 `409 vault_gas_insufficient`，前端引導自付 | [GasSponsorship.md](GasSponsorship.md) |

---

## Walrus 上傳 / 下載鏈路（[`storage.ts`](../../frontend/src/lib/storage.ts)）

- **上傳**：前端直接 `PUT {VITE_WALRUS_PUBLISHER_URL}/v1/blobs?epochs=N`；`N = VITE_WALRUS_STORAGE_EPOCHS`（程式 fallback **5**，根 `.env` 現設 **50**，應覆蓋問卷存活期 + purge 寬限期；Walrus 上限 `max_epochs_ahead = 53`）。回傳 `blobId` 與 `blobObjectId`。失敗時 fallback `POST {BFF}/api/storage/ipfs/upload`。
...
> **storage 補償已停用。** 原 `storage_compensation_amount`（claim 當下從 vault gas 池撥給自付 blob 答卷者）因 blob_id 鏈上不可驗、易被詐冒抽乾補助池而整條停用，無此金流；`SurveyVault` struct 已無此欄位，`create_empty` 仍保留同名參數位置但以 `let _ =` 忽略（保 ABI／PTB 相容）。答卷一律 inline。
...
- **僅問卷內容**走 Walrus（答卷一律 inline、無 blob）。Walrus blob **不會在 purge 時刪除**（由 epoch 到期自然過期）；`VITE_WALRUS_STORAGE_EPOCHS` 與 `purge_grace_ms` 需在部署時一起調校，避免問卷內容在存活期內已不可讀。**testnet caveat**：epoch 時長 ≈1 天且 Walrus `max_epochs_ahead = 53`，故 testnet 上 blob 最多約 53 天。
- 問卷內容 blob 的可用性與完整性由 `survey_blob_id` 上鏈背書；鏈上不驗 blob 內容雜湊（信任 Walrus content-addressing），但作答時前端 recompute `content_hash` 比對（見下）。
...
| `VITE_ANSWER_SIZE_THRESHOLD_KB` / `MAX_INLINE_ANSWER_KB` | 6 | 前端答卷 inline 上限（超過即阻擋送出；build 時對齊） |
| `MAX_INLINE_ANSWER_BYTES` / `MAX_INLINE_ANSWER_KB` | 6144 | BFF 代付 inline gate |
| `VITE_WALRUS_PUBLISHER_URL` / `VITE_WALRUS_AGGREGATOR_URL` | testnet 公共節點 | 前端問卷內容上傳/下載 |
| `WALRUS_AGGREGATOR_URL` | — | BFF walrus download proxy |
| `VITE_WALRUS_STORAGE_EPOCHS` | fallback 5 / `.env` 50 | 問卷內容 blob 存活 epoch 數（須 ≥ 問卷存活期 + 寬限期；受 `max_epochs_ahead=53` 上限） |
...
| 2026-06-15 | 新增「代付政策」：blob 答卷一律自付——前端上傳 Walrus 前先估 gas、跳自付確認，確認後 `forceSelfPaid` 送出；BFF 以 `SPONSOR_BLOB_ANSWERS_ENABLED`（預設關）防繞過。詳見 [GasSponsorship.md](GasSponsorship.md) |
| 2026-06-15 | **廢除答卷 Walrus 路線**：答卷一律 inline，超過上限即拒（禁止大型答卷）；移除 `answer_blob_id`／`max_blob_id_bytes`／`used_blob_ids` 與 `SPONSOR_BLOB_ANSWERS_ENABLED`；`storage_compensation_amount` 停用（struct 欄位已移除，`create_empty` 保留參數位置但忽略）。根因：blob_id 鏈上不可驗、易詐冒抽乾補助池。問卷內容 Walrus 不受影響 |
