...
V3 起，問卷可允許受訪者重複填答（`repeat_reward > 0`），但這不改變鏈上事實：
**每一次 `survey_vault::claim` 都會發出獨立的 `SurveyClaimed` 事件，所有版本的密文都永久保留在事件歷史**。

實務影響：

- **Dashboard「最新一次」顯示僅是前端過濾**，不代表舊版本從鏈上消失
- 發起人可重放所有歷史事件，**用 creator key 解出每位受訪者的所有答案版本**
- 受訪者若在填答後想「撤回」或「真正覆蓋舊答案」，鏈上機制無法支援
- 若需研究用途（追蹤同一人觀點演化），鏈上歷史本身就是來源；無需額外設計

CreatePage 與 SurveyPage 在啟用重複填答時應對受訪者揭露此點。
