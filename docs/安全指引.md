...
| `GAS_STATION_SHARED_SECRET` | **是** | BFF→Gas Station DO HMAC（`X-Gas-Station-Signature` + timestamp） | `bff/src/gas/gasStationClient.ts`, `workers/gas-station/src/gasStationDO.ts` |
| `GAS_STATION_MODE` | 否 | `local`（BFF 內建 pipeline）或 `do`（轉發 Worker）；`local` 不需 HMAC | `bff/src/gas/handler.ts` |
...
**代付 signer**：一律需 `GAS_SPONSOR_PRIV_1`、`GAS_SPONSOR_PRIV_2`、`GAS_SPONSOR_PUBKEY_3`（含本地開發）。`SURVEY_PASS_ISSUER_PRIV` **僅**簽 Pass ticket，不可兼作 gas sponsor。
...
| `CLOSE_TASK_ENABLED` | 到期自動關閉 OPEN vault（sponsor 代送 `close`） |
| `CLOSE_SCAN_INTERVAL_MS` / `CLOSE_MAX_PER_CYCLE` | Close 掃描參數（預設 12h / 10） |
| `PURGE_TASK_ENABLED` | 自動銷毀背景任務（關閉後 grace 期滿） |
...
| **Gas Station Worker**（`workers/gas-station/`） | `GAS_SPONSOR_PRIV_1/2`、`GAS_SPONSOR_PUBKEY_3`、`GAS_STATION_SHARED_SECRET`、`SURVEY_PASS_ISSUER_PRIV`（僅 DO 重驗 ticket）、`SUI_PACKAGE_ID`、Gas 設定 | Admin 私鑰 |
